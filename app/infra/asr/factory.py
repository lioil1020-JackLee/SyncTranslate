...
from app.infra.asr.streaming_pipeline import ASRManager as LegacyASRManager
from app.infra.config.schema import AppConfig


def normalize_asr_pipeline_mode(value: str) -> str:
    normalized = (value or "legacy").strip().lower()
    if normalized in {"legacy", "v1", "whisper_legacy"}:
        return "legacy"
    if normalized in {"v2", "next", "asr_v2"}:
        return "v2"
    return "legacy"
...
def create_asr_manager(
    ...
    *,
    ...
    mode = normalize_asr_pipeline_mode(str(getattr(config.runtime, "asr_pipeline", "legacy")))
    if mode == "v2":
        return ASRManagerV2(config, on_error=on_error, pipeline_revision=pipeline_revision)
    return LegacyASRManager(config, on_error=on_error, pipeline_revision=pipeline_revision)
