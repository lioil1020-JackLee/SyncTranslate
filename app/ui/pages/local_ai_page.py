...
_EDGE_VOICE_OPTIONS: list[tuple[str, str]] = [
    ("中文（台灣，女聲） - HsiaoChen", "zh-TW-HsiaoChenNeural"),
    ("中文（台灣，女聲） - HsiaoYu", "zh-TW-HsiaoYuNeural"),
    ("中文（台灣，男聲） - YunJhe", "zh-TW-YunJheNeural"),
    ("英文（美國，女聲） - Jenny", "en-US-JennyNeural"),
    ("英文（美國，男聲） - Guy", "en-US-GuyNeural"),
    ("英文（英國，女聲） - Sonia", "en-GB-SoniaNeural"),
    ("英文（英國，男聲） - Ryan", "en-GB-RyanNeural"),
    ("日文（日本，女聲） - Nanami", "ja-JP-NanamiNeural"),
    ("日文（日本，男聲） - Keita", "ja-JP-KeitaNeural"),
    ("韓文（韓國，女聲） - SunHi", "ko-KR-SunHiNeural"),
    ("韓文（韓國，男聲） - InJoon", "ko-KR-InJoonNeural"),
    ("泰文（泰國，女聲） - Premwadee", "th-TH-PremwadeeNeural"),
    ("泰文（泰國，男聲） - Niwat", "th-TH-NiwatNeural"),
]

_LLM_PROFILE_OPTIONS: list[tuple[str, str]] = [
    ("即時字幕（快速）", "live_caption_fast"),
    ("雙向對談（快速）", "dialogue_fast"),
...
_EXPERIENCE_PRESET_OPTIONS: list[tuple[str, str]] = [
    ("會議室監聽版", "meeting_monitor"),
    ("雙人對話版", "dialogue"),
]

_TTS_STYLE_OPTIONS: list[tuple[str, str]] = [
    ("平衡", "balanced"),
    ("播報清晰", "broadcast_clear"),
    ("對話自然", "conversational"),
    ("快速回應", "fast_response"),
]
...
class LocalAiPage(QWidget):
    def __init__(
        ...
        self.experience_group, experience_form = self._build_experience_group()
        ...
        self.base_tts = self._create_tts_widgets("TTS 璅∪?閮剖?")
        self.local_tts_override = self._create_tts_channel_widgets("?砍?孵?頛詨??閬神", "all")
        self.remote_tts_override = self._create_tts_channel_widgets("?垢?孵?頛詨??閬神", "all")
        self.tts_channel_group = self._build_tts_channel_compact_group()

        self.runtime_status_label = QLabel("系統就緒")
        ...
        content_stack.setSpacing(8)
        content_stack.addWidget(self.asr_group)
        content_stack.addWidget(self.runtime_group)
        content_stack.addWidget(self.llm_group)
        content_stack.addWidget(self.base_tts.group)
        content_stack.addWidget(self.tts_channel_group)
        content_stack.addStretch(1)
    ...
    def update_config(self, config: AppConfig) -> None:
        selected_preset = str(self.experience_preset_combo.currentData() or "meeting_monitor")
        zh_asr = config.asr_channels.local
        en_asr = config.asr_channels.remote
        ...
        if selected_preset == "dialogue":
            config.runtime.asr_profile_local = "turn_taking"
            config.runtime.asr_profile_remote = "turn_taking"
        else:
            config.runtime.asr_profile_local = "meeting_room"
            config.runtime.asr_profile_remote = "meeting_room"
        config.runtime.streaming_profile_local = "default"
        config.runtime.streaming_profile_remote = "default"
    ...
    def _apply_experience_preset(self, preset: str, *, notify: bool) -> None:
        ...
        try:
            ...
            self.runtime_tts_cancel_pending_check.setChecked(True)
            self.local_echo_guard_check.setChecked(False)
            ...
            if preset == "dialogue":
                ...
                self.remote_asr_partial_history_spin.setValue(1)
                ...
                self.asr_min_silence_spin.setValue(260)
                ...
                self.asr_temperature_fallback_local_edit.setText("0.0")
                self.asr_temperature_fallback_remote_edit.setText("0.0")
                self.asr_rms_threshold_spin.setValue(0.025)
                self.remote_asr_rms_threshold_spin.setValue(0.02)
                self.asr_no_speech_threshold_spin.setValue(0.45)
                self.remote_asr_no_speech_threshold_spin.setValue(0.40)
                self.asr_neural_threshold_spin.setValue(0.42)
                self.remote_asr_neural_threshold_spin.setValue(0.35)
                self.asr_soft_final_spin.setValue(1800)
                self.remote_asr_soft_final_spin.setValue(1600)
                self.asr_startup_suppress_spin.setValue(800)
                ...
                self.runtime_llm_partial_floor_spin.setValue(160)
                ...
                self.runtime_max_pipeline_latency_spin.setValue(1800)
                ...
                self.asr_final_history_spin.setValue(20)
                ...
                self.asr_min_silence_spin.setValue(1000)
                self.remote_asr_min_silence_spin.setValue(600)
                self.asr_speech_pad_spin.setValue(2000)
                ...
                self.asr_temperature_fallback_local_edit.setText("0.0")
                self.asr_temperature_fallback_remote_edit.setText("0.0,0.2,0.4")
                self.asr_rms_threshold_spin.setValue(0.025)
                self.remote_asr_rms_threshold_spin.setValue(0.02)
                self.asr_no_speech_threshold_spin.setValue(0.65)
                self.remote_asr_no_speech_threshold_spin.setValue(0.35)
                self.asr_neural_threshold_spin.setValue(0.55)
                self.remote_asr_neural_threshold_spin.setValue(0.30)
                self.asr_soft_final_spin.setValue(8000)
                self.remote_asr_soft_final_spin.setValue(3000)
                self.asr_startup_suppress_spin.setValue(3000)
                self.remote_asr_startup_suppress_spin.setValue(0)
                self.asr_hallucination_filter_check.setChecked(True)
                self.remote_asr_hallucination_filter_check.setChecked(False)
    ...
    def _update_preset_labels(self, preset: str) -> None:
        if preset == "dialogue":
            self.channel_strategy_label.setText("雙向短句優先，會提早切段、縮短翻譯上下文，讓來回對話更跟得上。")
            self.optimized_profile_label.setText("適合一來一往對談、口譯協助、短句即時翻譯。")
            return
        self.channel_strategy_label.setText("會議穩定度優先，保留較長上下文與較保守切段，較適合監聽和字幕。")
        self.optimized_profile_label.setText("適合會議監聽、長句發言、較重視穩定度的場景。")
    ...
    def _build_experience_group(self) -> tuple[QGroupBox, QFormLayout]:
        ...
        self.show_advanced_check.setChecked(False)
        self.channel_strategy_label = QLabel("會議穩定度優先，保留較長上下文與較保守切段，較適合監聽和字幕。")
        self.optimized_profile_label = QLabel("適合會議監聽、長句發言、較重視穩定度的場景。")
        ...
        self.asr_concurrency_summary_label = QLabel(
            "裝置/精度會同時套用到兩個 ASR 通道。若 CUDA 可用會使用 GPU，否則退回 CPU。"
        )
        ...
        form.addRow("運算提醒", self.asr_concurrency_summary_label)
    ...
    def _set_advanced_settings_visible(self, visible: bool) -> None:
        self.asr_group.setVisible(visible)
        self.llm_group.setVisible(visible)
        self.runtime_group.setVisible(visible)
        self.base_tts.group.setVisible(visible)
        self.tts_channel_group.setVisible(visible)

    def _apply_channel_strategy_controls(self) -> None:
        for widget in (
            ...
            self.remote_asr_neural_threshold_spin,
            self.remote_asr_soft_final_spin,
            self.remote_asr_startup_suppress_spin,
            self.remote_asr_hallucination_filter_check,
    ...
    def _build_asr_group(self) -> tuple[QGroupBox, QFormLayout]:
        ...
        self.asr_device_combo.setToolTip("設定 ASR 的運算裝置；可用 CUDA 時會顯示 GPU，否則使用 CPU。")
        ...
        self.asr_min_silence_spin.setRange(120, 8000)
        self.remote_asr_min_silence_spin = QSpinBox()
        self.remote_asr_min_silence_spin.setRange(120, 8000)
        ...
        self.asr_neural_threshold_spin = QDoubleSpinBox()
        self.asr_neural_threshold_spin.setRange(0.05, 0.95)
        self.asr_neural_threshold_spin.setSingleStep(0.05)
        self.asr_neural_threshold_spin.setDecimals(2)
        self.asr_neural_threshold_spin.setToolTip("Silero VAD 神經網路觸發門檻，越高越不容易觸發語音偵測。")
        self.remote_asr_neural_threshold_spin = QDoubleSpinBox()
        self.remote_asr_neural_threshold_spin.setRange(0.05, 0.95)
        self.remote_asr_neural_threshold_spin.setSingleStep(0.05)
        self.remote_asr_neural_threshold_spin.setDecimals(2)
        self.remote_asr_neural_threshold_spin.setToolTip("Silero VAD 神經網路觸發門檻，越高越不容易觸發語音偵測。")
        self.asr_soft_final_spin = QSpinBox()
        self.asr_soft_final_spin.setRange(500, 12000)
        self.asr_soft_final_spin.setSingleStep(500)
        self.asr_soft_final_spin.setToolTip("語音靜音後延遲此時間再觸發 Final，避免句子被切斷。")
        self.remote_asr_soft_final_spin = QSpinBox()
        self.remote_asr_soft_final_spin.setRange(500, 12000)
        self.remote_asr_soft_final_spin.setSingleStep(500)
        self.remote_asr_soft_final_spin.setToolTip("語音靜音後延遲此時間再觸發 Final，避免句子被切斷。")
        self.asr_startup_suppress_spin = QSpinBox()
        self.asr_startup_suppress_spin.setRange(0, 10000)
        self.asr_startup_suppress_spin.setSingleStep(500)
        self.asr_startup_suppress_spin.setToolTip("啟動後此時間內忽略語音觸發，避免片頭音樂/雜音產生幻覺（0 = 停用）。")
        self.remote_asr_startup_suppress_spin = QSpinBox()
        self.remote_asr_startup_suppress_spin.setRange(0, 10000)
        self.remote_asr_startup_suppress_spin.setSingleStep(500)
        self.remote_asr_startup_suppress_spin.setToolTip("啟動後此時間內忽略語音觸發，避免片頭音樂/雜音產生幻覺（0 = 停用）。")
        self.asr_hallucination_filter_check = QCheckBox("幻覺過濾")
        self.asr_hallucination_filter_check.setToolTip("過濾 Whisper 常見幻覺輸出（作詞作曲、字幕志願者、純標點等）。")
        self.remote_asr_hallucination_filter_check = QCheckBox("幻覺過濾")
        self.remote_asr_hallucination_filter_check.setToolTip("過濾 Whisper 常見幻覺輸出（作詞作曲、字幕志願者、純標點等）。")
        ...
        self._set_dual_field_style(
            ...
            self.asr_temperature_fallback_remote_edit,
            self.asr_neural_threshold_spin,
            self.remote_asr_neural_threshold_spin,
            self.asr_soft_final_spin,
            self.remote_asr_soft_final_spin,
            self.asr_startup_suppress_spin,
            self.remote_asr_startup_suppress_spin,
            self.asr_hallucination_filter_check,
            self.remote_asr_hallucination_filter_check,
        )
        ...
        form.addRow("VAD 神經門檻", self._build_dual_field_row(self.asr_neural_threshold_spin, self.remote_asr_neural_threshold_spin))
        form.addRow("Soft Final(ms)", self._build_dual_field_row(self.asr_soft_final_spin, self.remote_asr_soft_final_spin))
        form.addRow("啟動抑制(ms)", self._build_dual_field_row(self.asr_startup_suppress_spin, self.remote_asr_startup_suppress_spin))
        zh_halluc_row = self._build_inline_row(self.asr_hallucination_filter_check)
        en_halluc_row = self._build_inline_row(self.remote_asr_hallucination_filter_check)
        form.addRow("幻覺過濾", self._build_dual_field_row(zh_halluc_row, en_halluc_row))
    ...
    def _build_llm_group(self) -> tuple[QGroupBox, QFormLayout]:
        ...
        self.llm_model_label.setToolTip("本地方向翻譯模型固定。")
        self.remote_llm_model_label.setToolTip("遠端方向翻譯模型固定。")
        ...
        self.llm_sliding_window_enabled = QCheckBox("使用 partial 滑動視窗")
        self.remote_llm_sliding_window_enabled = QCheckBox("使用 partial 滑動視窗")
        ...
        form.addRow("皞怠漲", self._build_dual_field_row(self.llm_temperature_spin, self.remote_llm_temperature_spin))
        ...
        form.addRow("使用 partial 滑動視窗", self._build_dual_field_row(self.llm_sliding_window_enabled, self.remote_llm_sliding_window_enabled))
        form.addRow("閫貊 Token", self._build_dual_field_row(self.llm_trigger_tokens_spin, self.remote_llm_trigger_tokens_spin))
        form.addRow("上下文保留筆數", self._build_dual_field_row(self.llm_context_items_spin, self.remote_llm_context_items_spin))
    ...
    def _build_runtime_group(self) -> tuple[QGroupBox, QFormLayout]:
        ...
        self.runtime_tts_accept_stable_partial_check = QCheckBox("TTS 僅接受穩定 partial")
        self.runtime_tts_partial_min_chars_spin = QSpinBox()
        self.runtime_tts_partial_min_chars_spin.setRange(1, 200)
        self.runtime_tts_use_speech_profile_check = QCheckBox("TTS 啟用 speech profile")
        self.runtime_tts_cancel_pending_check = QCheckBox("新句到達時取消待播 TTS")
        self.runtime_tts_cancel_policy_combo = QComboBox()
        self.runtime_tts_cancel_policy_combo.addItem("取消全部待播", "all_pending")
        self.runtime_tts_cancel_policy_combo.addItem("只取消較舊待播", "older_only")
        ...
        self.local_echo_guard_check = QCheckBox("啟用回聲防護")
        ...
        form = QFormLayout()
        self._configure_form_layout(form)
        form.addRow("系統取樣率", self.runtime_sample_rate_spin)
        form.addRow("延遲模式", self.runtime_latency_mode_combo)
        form.addRow("顯示 partial 策略", self.runtime_display_partial_strategy_combo)
        form.addRow("音訊分塊(ms)", self.runtime_chunk_spin)
        form.addRow("ASR Pre-roll(ms)", self.runtime_asr_pre_roll_spin)
        form.addRow("穩定 partial 重複次數", self.runtime_stable_partial_min_repeats_spin)
        form.addRow("穩定 partial 最大字元差", self.runtime_partial_stability_delta_spin)
        form.addRow("ASR partial 最小音訊長(ms)", self.runtime_asr_partial_min_audio_spin)
        form.addRow("ASR partial 最短延遲(ms)", self.runtime_asr_partial_floor_spin)
        form.addRow("LLM partial 最短延遲(ms)", self.runtime_llm_partial_floor_spin)
        ...
        form.addRow("啟用回聲防護", self.local_echo_guard_check)
        form.addRow("回聲防護啟動延遲(ms)", self.local_echo_guard_delay_spin)
        form.addRow("遠端回聲防護啟動延遲(ms)", self.remote_echo_guard_delay_spin)
        form.addRow("翻譯快取大小", self.runtime_translation_cache_spin)
        form.addRow("前綴增量最小字數", self.runtime_prefix_delta_spin)
        ...
        group = QGroupBox("執行期設定")
        group.setLayout(form)
        return group, form

    def _create_tts_widgets(self, title: str) -> TtsWidgets:
        engine_combo = QComboBox()
        engine_combo.addItem("Edge TTS（內建）", "edge_tts")
        ...
        form = QFormLayout()
        self._configure_form_layout(form)
        form.addRow("??憸冽", style_preset_combo)
        form.addRow("隤?Length Scale", length_scale_spin)
        form.addRow("?冽?摨?Noise Scale", noise_scale_spin)
        form.addRow("?唾 Noise W", noise_w_spin)
        form.addRow("取樣率", sample_rate_spin)
        form.addRow("TTS 丟棄積壓門檻", self.runtime_tts_drop_threshold_spin)
        form.addRow("TTS ?憭抒?敺?ms)", self.runtime_tts_max_wait_spin)
        form.addRow("TTS 最大字數", self.runtime_tts_max_chars_spin)
        form.addRow("??蝑", self.runtime_tts_cancel_policy_combo)
        form.addRow("", self.runtime_tts_cancel_pending_check)
    ...
    def _edge_voice_options_for(language_kind: str) -> list[tuple[str, str]]:
        ...
        if normalized in {"all", "*"}:
            return list(_EDGE_VOICE_OPTIONS)
        if normalized.startswith("en"):
            return [item for item in _EDGE_VOICE_OPTIONS if item[1].lower().startswith("en-")]
        if normalized.startswith("ja"):
            return [item for item in _EDGE_VOICE_OPTIONS if item[1].lower().startswith("ja-")]
        if normalized.startswith("ko"):
            return [item for item in _EDGE_VOICE_OPTIONS if item[1].lower().startswith("ko-")]
        if normalized.startswith("th"):
            return [item for item in _EDGE_VOICE_OPTIONS if item[1].lower().startswith("th-")]
    ...
    def _wire_events(self) -> None:
        ...
        self._bind_tts_widget_events(self.base_tts, "基礎 TTS")
    ...
    def _load_speaker_rows_from_tts_config(self, config_path_text: str) -> list[tuple[str, int]]:
        ...
        if isinstance(speaker_map, dict):
            ...
            for name, sid in speaker_map.items():
                try:
                    rows.append((f"說話人：{name}", int(sid)))
                except Exception:
                    continue
    ...
    def _on_experience_preset_changed(self) -> None:
        ...
        self._apply_experience_preset(preset, notify=True)
    ...
    def eventFilter(self, watched, event):  # type: ignore[override]
        if event.type() == QEvent.Type.Wheel:
            if isinstance(watched, QComboBox) or isinstance(watched, QAbstractSpinBox):
                return True
    ...
    @staticmethod
    def _na_label(text: str = "-") -> QLabel:
        label = QLabel(text)
        label.setStyleSheet("color: #9aa0a6;")
        return label

    @staticmethod
    def _set_uniform_field_style(widget: QWidget, minimum_width: int = 180, control_height: int = _CONTROL_HEIGHT) -> None:
    ...
    @staticmethod
    def _set_multiline_field_style(widget: QPlainTextEdit, minimum_width: int = 180, control_height: int = 72) -> None:
        widget.setMinimumWidth(minimum_width)
        widget.setMinimumHeight(control_height)
        widget.setMaximumHeight(control_height)
        widget.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
