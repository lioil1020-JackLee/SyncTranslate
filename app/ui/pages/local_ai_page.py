...
class LocalAiPage(QWidget):
    def __init__(
        ...
        self.base_tts = self._create_tts_widgets("TTS 基礎設定")
        self.local_tts_override = self._create_tts_channel_widgets("本地方向語音輸出覆寫", "all")
        self.remote_tts_override = self._create_tts_channel_widgets("遠端方向語音輸出覆寫", "all")
    ...
    def _create_tts_widgets(self, title: str) -> TtsWidgets:
        ...
        form = QFormLayout()
        self._configure_form_layout(form)
        form.addRow("語音風格", style_preset_combo)
        form.addRow("語速 / Length Scale", length_scale_spin)
        form.addRow("語音隨機度 / Noise Scale", noise_scale_spin)
        form.addRow("韻律變化 / Noise W", noise_w_spin)
