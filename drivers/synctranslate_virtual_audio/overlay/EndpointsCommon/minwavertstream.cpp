...
// SyncTranslate Phase 3 bridge contract: 64-byte header + float32 PCM payload.
#define SYNC_RINGBUF_HEADER_BYTES 64u
#define SYNC_BRIDGE_MAX_SESSION_ID 32u

typedef struct _SYNC_RINGBUF_HEADER
{
    LONGLONG capacity_frames;
    LONGLONG channels;
    LONGLONG write_pos;
    LONGLONG buffered_frames;
    LONGLONG total_written_frames;
    LONGLONG dropped_frames;
    BYTE reserved[16];
} SYNC_RINGBUF_HEADER;

typedef struct _SYNC_RINGBUF_VIEW
{
    HANDLE sectionHandle;
    HANDLE eventHandle;
    PVOID baseAddress;
    SIZE_T viewSize;
    volatile SYNC_RINGBUF_HEADER* header;
    volatile FLOAT* payload;
    BOOLEAN initialized;
} SYNC_RINGBUF_VIEW;

static SYNC_RINGBUF_VIEW g_syncRemoteInput = { 0 };
static SYNC_RINGBUF_VIEW g_syncVirtualMic = { 0 };
static volatile LONGLONG g_syncBridgeHeartbeatTickMs = 0;
static volatile LONG g_syncBridgeHeartbeatAlive = 1;

#define SYNC_BRIDGE_KERNEL_ENABLED 0
// Keep the kernel TTS injection path disabled by default.
// It previously caused host BSODs in tabletaudiosample.sys; future work must
// validate any replacement path in a disposable VM before enabling this flag.
#define SYNC_TTS_INJECTION_ENABLED 0
#define SYNC_DRIVER_PCM_RING_ENABLED 1
#define SYNC_TTS_INJECTION_PIN KSPIN_WAVE_RENDER_SINK_OFFLOAD
#define SYNC_TTS_LOOPBACK_CAPACITY_FRAMES (48000u * 10u)

#if SYNC_TTS_INJECTION_ENABLED
typedef struct _SYNC_TTS_LOOPBACK_BUFFER
{
    KSPIN_LOCK lock;
    volatile LONG initState;
    LONGLONG readPos;
    LONGLONG writePos;
    LONGLONG bufferedFrames;
    LONGLONG totalWrittenFrames;
    LONGLONG droppedFrames;
    FLOAT samples[SYNC_TTS_LOOPBACK_CAPACITY_FRAMES];
} SYNC_TTS_LOOPBACK_BUFFER;

static SYNC_TTS_LOOPBACK_BUFFER g_syncTtsLoopback = { 0 };

static VOID SyncTtsLoopbackEnsureInitialized()
{
    LONG state = InterlockedCompareExchange(&g_syncTtsLoopback.initState, 1, 0);
    if (state == 0)
    {
        KeInitializeSpinLock(&g_syncTtsLoopback.lock);
        g_syncTtsLoopback.readPos = 0;
        g_syncTtsLoopback.writePos = 0;
        g_syncTtsLoopback.bufferedFrames = 0;
        g_syncTtsLoopback.totalWrittenFrames = 0;
        g_syncTtsLoopback.droppedFrames = 0;
        RtlZeroMemory(g_syncTtsLoopback.samples, sizeof(g_syncTtsLoopback.samples));
        InterlockedExchange(&g_syncTtsLoopback.initState, 2);
        return;
    }

    while (InterlockedCompareExchange(&g_syncTtsLoopback.initState, 2, 2) != 2)
    {
        KeStallExecutionProcessor(1);
    }
}

static ULONG SyncTtsBytesPerSample(_In_ const WAVEFORMATEXTENSIBLE* wfExt)
{
...
static FLOAT SyncTtsReadSampleAsFloat(
    _In_reads_bytes_(sampleBytes) const BYTE* src,
    _In_ ULONG sampleBytes,
    _In_ USHORT formatTag,
    _In_ const GUID* subFormat
)
{
    if (sampleBytes == sizeof(FLOAT) && (formatTag == WAVE_FORMAT_IEEE_FLOAT ||
        (formatTag == WAVE_FORMAT_EXTENSIBLE && subFormat != NULL && IsEqualGUID(*subFormat, KSDATAFORMAT_SUBTYPE_IEEE_FLOAT))))
    {
    ...
    {
        return (FLOAT)(*(const SHORT*)src) / 32768.0f;
    }

    return 0.0f;
}

static VOID SyncTtsWriteSampleFromFloat(
...
static VOID SyncTtsLoopbackWriteFromDma(
    _In_ const WAVEFORMATEXTENSIBLE* wfExt,
    _In_reads_bytes_(bytes) const BYTE* src,
    _In_ ULONG bytes
)
{
    if (wfExt == NULL || src == NULL || bytes == 0)
    {
        return;
    }

    ULONG channels = max(1u, wfExt->Format.nChannels);
    ULONG sampleBytes = SyncTtsBytesPerSample(wfExt);
    if (sampleBytes == 0)
    {
        return;
    }
    ...
    SyncTtsLoopbackEnsureInitialized();
    KIRQL oldIrql;
    KeAcquireSpinLock(&g_syncTtsLoopback.lock, &oldIrql);
    for (ULONG frame = 0; frame < frames; ++frame)
    {
        const BYTE* framePtr = src + ((SIZE_T)frame * bytesPerFrame);
        FLOAT mixed = 0.0f;
        for (ULONG channel = 0; channel < channels; ++channel)
        {
            mixed += SyncTtsReadSampleAsFloat(framePtr + ((SIZE_T)channel * sampleBytes), sampleBytes, formatTag, subFormat);
        }
        mixed = mixed / (FLOAT)channels;

        if (g_syncTtsLoopback.bufferedFrames >= SYNC_TTS_LOOPBACK_CAPACITY_FRAMES)
        {
            g_syncTtsLoopback.readPos = (g_syncTtsLoopback.readPos + 1) % SYNC_TTS_LOOPBACK_CAPACITY_FRAMES;
            g_syncTtsLoopback.bufferedFrames--;
            g_syncTtsLoopback.droppedFrames++;
        }

        g_syncTtsLoopback.samples[g_syncTtsLoopback.writePos] = mixed;
        g_syncTtsLoopback.writePos = (g_syncTtsLoopback.writePos + 1) % SYNC_TTS_LOOPBACK_CAPACITY_FRAMES;
        g_syncTtsLoopback.bufferedFrames++;
        g_syncTtsLoopback.totalWrittenFrames++;
    }
    KeReleaseSpinLock(&g_syncTtsLoopback.lock, oldIrql);
}

static VOID SyncTtsLoopbackReadIntoDma(
    _In_ const WAVEFORMATEXTENSIBLE* wfExt,
...
)
{
    ...
    {
        FLOAT sample = 0.0f;
        if (g_syncTtsLoopback.bufferedFrames > 0)
        {
            sample = g_syncTtsLoopback.samples[g_syncTtsLoopback.readPos];
            g_syncTtsLoopback.readPos = (g_syncTtsLoopback.readPos + 1) % SYNC_TTS_LOOPBACK_CAPACITY_FRAMES;
            g_syncTtsLoopback.bufferedFrames--;
        }
        ...
        for (ULONG channel = 0; channel < channels; ++channel)
        {
            SyncTtsWriteSampleFromFloat(framePtr + ((SIZE_T)channel * sampleBytes), sampleBytes, sample, formatTag, subFormat);
        }
    }
    KeReleaseSpinLock(&g_syncTtsLoopback.lock, oldIrql);
}
#endif // SYNC_TTS_INJECTION_ENABLED

#if SYNC_BRIDGE_KERNEL_ENABLED
static BOOLEAN SyncBridgeTryGetLocalObjectName(_In_ PCWSTR objectPath, _Outptr_ PCWSTR* localObjectName)
{
    static const WCHAR prefix[] = L"\\BaseNamedObjects\\Local\\";
    SIZE_T i = 0;

    if (objectPath == NULL || localObjectName == NULL)
    {
        return FALSE;
    }
    ...
    *localObjectName = objectPath + i;
    return TRUE;
}

static NTSTATUS SyncBridgeOpenSection(
    _In_ PCWSTR sectionName,
    _Out_ HANDLE* sectionHandle,
    _Out_opt_ ULONG* sessionId
)
{
    ...
    *sectionHandle = NULL;
    ...
    RtlInitUnicodeString(&sectionPath, sectionName);
    InitializeObjectAttributes(&sectionAttr, &sectionPath, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);

    NTSTATUS status = ZwOpenSection(sectionHandle, SECTION_MAP_READ | SECTION_MAP_WRITE, &sectionAttr);
    if (NT_SUCCESS(status))
    {
        return status;
    }

    PCWSTR localObjectName = NULL;
    if (!SyncBridgeTryGetLocalObjectName(sectionName, &localObjectName))
    {
        return status;
    }

    for (ULONG currentSessionId = 0; currentSessionId < SYNC_BRIDGE_MAX_SESSION_ID; ++currentSessionId)
    {
        WCHAR sessionPathBuffer[160] = { 0 };
        status = RtlStringCchPrintfW(
            sessionPathBuffer,
            RTL_NUMBER_OF(sessionPathBuffer),
            L"\\Sessions\\%lu\\BaseNamedObjects\\%ws",
            currentSessionId,
            localObjectName
        );
        if (!NT_SUCCESS(status))
        ...
        RtlInitUnicodeString(&sectionPath, sessionPathBuffer);
        InitializeObjectAttributes(&sectionAttr, &sectionPath, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
        status = ZwOpenSection(sectionHandle, SECTION_MAP_READ | SECTION_MAP_WRITE, &sectionAttr);
        if (NT_SUCCESS(status))
        {
            if (sessionId != NULL)
            {
...
static HANDLE SyncBridgeOpenEvent(_In_opt_ PCWSTR eventName, _In_ ULONG sessionId)
{
    if (eventName == NULL)
    {
        return NULL;
    ...
    RtlInitUnicodeString(&eventPath, eventName);
    InitializeObjectAttributes(&eventAttr, &eventPath, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);

    HANDLE eventHandle = NULL;
    NTSTATUS status = ZwOpenEvent(&eventHandle, EVENT_MODIFY_STATE | SYNCHRONIZE, &eventAttr);
    if (NT_SUCCESS(status))
    {
        return eventHandle;
    }
    ...
    PCWSTR localObjectName = NULL;
    if (sessionId == ULONG_MAX || !SyncBridgeTryGetLocalObjectName(eventName, &localObjectName))
    {
        return NULL;
    }
    ...
    RtlInitUnicodeString(&eventPath, sessionPathBuffer);
    InitializeObjectAttributes(&eventAttr, &eventPath, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
    status = ZwOpenEvent(&eventHandle, EVENT_MODIFY_STATE | SYNCHRONIZE, &eventAttr);
    return NT_SUCCESS(status) ? eventHandle : NULL;
}

static BOOLEAN SyncBridgePingPipeAlive()
{
    if (KeGetCurrentIrql() > PASSIVE_LEVEL)
    {
        // Named pipe probing is passive-level only; use the cached heartbeat from higher IRQL.
        return (g_syncBridgeHeartbeatAlive != 0);
    }

    UNICODE_STRING pipePath;
    OBJECT_ATTRIBUTES pipeAttr;
    IO_STATUS_BLOCK ioStatus;
    RtlInitUnicodeString(&pipePath, L"\\Device\\NamedPipe\\SyncTranslateAudioBridge");
    InitializeObjectAttributes(&pipeAttr, &pipePath, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
...
static BOOLEAN SyncBridgeHeartbeatOk()
{
    LONGLONG nowMs = (LONGLONG)(KeQueryInterruptTime() / 10000);
    LONGLONG lastMs = InterlockedCompareExchange64(&g_syncBridgeHeartbeatTickMs, 0, 0);
    if (lastMs > 0 && (nowMs - lastMs) < 100)
    {
        return (g_syncBridgeHeartbeatAlive != 0);
    }

    BOOLEAN alive = SyncBridgePingPipeAlive();
    InterlockedExchange64(&g_syncBridgeHeartbeatTickMs, nowMs);
    InterlockedExchange(&g_syncBridgeHeartbeatAlive, alive ? 1 : 0);
    return alive;
}

static VOID SyncBridgeSetEvent(_In_ HANDLE eventHandle)
{
    if (eventHandle != NULL)
    {
        ...
        if (NT_SUCCESS(status) && eventObject != NULL)
        {
...
static VOID SyncBridgeResetEvent(_In_ HANDLE eventHandle)
{
    if (eventHandle != NULL)
    {
        PKEVENT eventObject = NULL;
...
static NTSTATUS SyncBridgeEnsureView(
    _Inout_ SYNC_RINGBUF_VIEW* view,
    _In_ PCWSTR sectionName,
...
)
{
    ...
    NTSTATUS status = SyncBridgeOpenSection(sectionName, &sectionHandle, &sessionId);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    ...
    if (viewSize < SYNC_RINGBUF_HEADER_BYTES)
    {
    ...
    if (eventName != NULL)
    {
        eventHandle = SyncBridgeOpenEvent(eventName, sessionId);
    }

    view->sectionHandle = sectionHandle;
    view->eventHandle = eventHandle;
    view->baseAddress = baseAddress;
    view->viewSize = viewSize;
    view->header = (volatile SYNC_RINGBUF_HEADER*)baseAddress;
    view->payload = (volatile FLOAT*)((PUCHAR)baseAddress + SYNC_RINGBUF_HEADER_BYTES);
    view->initialized = TRUE;
    return STATUS_SUCCESS;
}

static ULONG SyncBytesPerSample(_In_ const WAVEFORMATEXTENSIBLE* wfExt)
{
    if (wfExt == NULL || wfExt->Format.nBlockAlign == 0)
    {
        return 0;
...
static FLOAT SyncReadSampleAsFloat(
    _In_reads_bytes_(sampleBytes) const BYTE* src,
    _In_ ULONG sampleBytes,
    _In_ USHORT formatTag,
    _In_ const GUID* subFormat
...
static VOID SyncWriteSampleFromFloat(
    _Out_writes_bytes_(sampleBytes) BYTE* dst,
    _In_ ULONG sampleBytes,
    _In_ FLOAT value,
    _In_ USHORT formatTag,
...
static NTSTATUS SyncBridgeWriteFromDma(
    _In_ const WAVEFORMATEXTENSIBLE* wfExt,
    _In_reads_bytes_(bytes) const BYTE* src,
    _In_ ULONG bytes
)
{
    ...
    // Policy: virtual speaker pass-through is handled by the normal endpoint
    // rendering path. If bridge heartbeat fails, skip ring-buffer ingestion so
    // stale data is not accumulated in remote_input.
    if (!SyncBridgeHeartbeatOk())
    {
        return STATUS_DEVICE_NOT_READY;
    }

    NTSTATUS status = SyncBridgeEnsureView(
        &g_syncRemoteInput,
        L"\\BaseNamedObjects\\Local\\SyncTranslateRemoteInput",
        L"\\BaseNamedObjects\\Local\\SyncTranslateRemoteInputReady"
    );
    if (!NT_SUCCESS(status))
    {
        return status;
    ...
    volatile SYNC_RINGBUF_HEADER* h = g_syncRemoteInput.header;
    if (h == NULL || h->capacity_frames <= 0 || h->channels <= 0)
    {
        return STATUS_INVALID_DEVICE_STATE;
    }

    ULONG srcChannels = max(1u, wfExt->Format.nChannels);
    ULONG srcSampleBytes = SyncBytesPerSample(wfExt);
    if (srcSampleBytes == 0)
    {
        return STATUS_INVALID_PARAMETER;
    }

    ULONG bytesPerFrame = srcSampleBytes * srcChannels;
    if (bytesPerFrame == 0)
    {
        return STATUS_INVALID_PARAMETER;
    }
    ULONG srcFrames = bytes / bytesPerFrame;
    if (srcFrames == 0)
    {
        return STATUS_SUCCESS;
    }

    LONGLONG cap = h->capacity_frames;
    LONGLONG dstChannels = h->channels;
    LONGLONG writePos = h->write_pos;
    LONGLONG buffered = h->buffered_frames;
    LONGLONG totalWritten = h->total_written_frames;
    LONGLONG dropped = h->dropped_frames;

    LONGLONG frames = (LONGLONG)srcFrames;
    LONGLONG srcStartFrame = 0;
    if (frames > cap)
    {
        srcStartFrame = frames - cap;
        dropped += (frames - cap);
        frames = cap;
    }
    else
    {
        LONGLONG overflow = buffered + frames - cap;
        if (overflow > 0)
        {
            dropped += overflow;
        }
    ...
    USHORT formatTag = wfExt->Format.wFormatTag;
    const GUID* subFormat = (formatTag == WAVE_FORMAT_EXTENSIBLE) ? &wfExt->SubFormat : NULL;
    for (LONGLONG i = 0; i < frames; ++i)
    {
        LONGLONG frameIndex = srcStartFrame + i;
        const BYTE* framePtr = src + frameIndex * bytesPerFrame;
        LONGLONG dstFrame = (writePos + i) % cap;
        for (LONGLONG c = 0; c < dstChannels; ++c)
        {
            ULONG srcChannel = (ULONG)min((LONGLONG)(srcChannels - 1), c);
            const BYTE* samplePtr = framePtr + srcChannel * srcSampleBytes;
            g_syncRemoteInput.payload[dstFrame * dstChannels + c] = SyncReadSampleAsFloat(samplePtr, srcSampleBytes, formatTag, subFormat);
        }
    }

    h->write_pos = (writePos + frames) % cap;
    h->buffered_frames = min(cap, buffered + frames);
    h->total_written_frames = totalWritten + frames;
    h->dropped_frames = dropped;
    SyncBridgeSetEvent(g_syncRemoteInput.eventHandle);
    return STATUS_SUCCESS;
}

static NTSTATUS SyncBridgeReadIntoDma(
    _In_ const WAVEFORMATEXTENSIBLE* wfExt,
    _Out_writes_bytes_(bytes) BYTE* dst,
    _In_ ULONG bytes
)
{
    ...
    // Driver silence fallback: when bridge heartbeat is unhealthy, keep
    // emitting silence to the virtual microphone capture stream.
    if (!SyncBridgeHeartbeatOk())
    {
        RtlZeroMemory(dst, bytes);
        SyncBridgeResetEvent(g_syncVirtualMic.eventHandle);
        return STATUS_DEVICE_NOT_READY;
    }

    NTSTATUS status = SyncBridgeEnsureView(
        &g_syncVirtualMic,
        L"\\BaseNamedObjects\\Local\\SyncTranslateVirtualMicrophoneBuffer",
        L"\\BaseNamedObjects\\Local\\SyncTranslateVirtualMicrophoneReady"
    );
    if (!NT_SUCCESS(status))
    {
        RtlZeroMemory(dst, bytes);
    ...
    volatile SYNC_RINGBUF_HEADER* h = g_syncVirtualMic.header;
    if (h == NULL || h->capacity_frames <= 0 || h->channels <= 0)
    {
        RtlZeroMemory(dst, bytes);
        return STATUS_INVALID_DEVICE_STATE;
    ...
    ULONG dstChannels = max(1u, wfExt->Format.nChannels);
    ULONG dstSampleBytes = SyncBytesPerSample(wfExt);
    if (dstSampleBytes == 0)
    {
        RtlZeroMemory(dst, bytes);
        return STATUS_INVALID_PARAMETER;
    ...
    ULONG bytesPerFrame = dstSampleBytes * dstChannels;
    if (bytesPerFrame == 0)
    {
        RtlZeroMemory(dst, bytes);
        return STATUS_INVALID_PARAMETER;
    }
    ULONG dstFrames = bytes / bytesPerFrame;
    if (dstFrames == 0)
    {
        return STATUS_SUCCESS;
    }
    ...
    LONGLONG cap = h->capacity_frames;
    LONGLONG srcChannels = h->channels;
    LONGLONG writePos = h->write_pos;
    LONGLONG buffered = h->buffered_frames;
    LONGLONG available = min((LONGLONG)dstFrames, buffered);
    LONGLONG start = (writePos - buffered) % cap;
    if (start < 0)
    {
        start += cap;
    }
    ...
    {
        BYTE* framePtr = dst + ((SIZE_T)i * bytesPerFrame);
        BOOLEAN haveData = ((LONGLONG)i < available);
        LONGLONG srcFrame = haveData ? ((start + i) % cap) : 0;
        for (ULONG c = 0; c < dstChannels; ++c)
        {
            FLOAT sample = 0.0f;
            if (haveData)
            {
                LONGLONG srcChannel = min((LONGLONG)c, srcChannels - 1);
                sample = g_syncVirtualMic.payload[srcFrame * srcChannels + srcChannel];
            }
            SyncWriteSampleFromFloat(framePtr + ((SIZE_T)c * dstSampleBytes), dstSampleBytes, sample, formatTag, subFormat);
        }
    }

    h->buffered_frames = max(0LL, buffered - available);
    if (h->buffered_frames > 0)
    {
        SyncBridgeSetEvent(g_syncVirtualMic.eventHandle);
    }
    else
    {
        SyncBridgeResetEvent(g_syncVirtualMic.eventHandle);
...
#endif // SYNC_BRIDGE_KERNEL_ENABLED
...
#if SYNC_DRIVER_PCM_RING_ENABLED
        ntStatus = SyncTranslatePcmRingInitialize(SYNCTRANSLATE_PCM_RING_DEFAULT_FRAMES);
        if (!NT_SUCCESS(ntStatus) && ntStatus != STATUS_DEVICE_BUSY)
        {
            return ntStatus;
        }
#endif
...
#if SYNC_BRIDGE_KERNEL_ENABLED
            if (SYNC_BRIDGE_KERNEL_ENABLED && m_bCapture)
            {
                (void)SyncBridgeEnsureView(
                    &g_syncVirtualMic,
                    L"\\BaseNamedObjects\\Local\\SyncTranslateVirtualMicrophoneBuffer",
                    L"\\BaseNamedObjects\\Local\\SyncTranslateVirtualMicrophoneReady"
                );
            }
            else if (SYNC_BRIDGE_KERNEL_ENABLED)
            {
                (void)SyncBridgeEnsureView(
                    &g_syncRemoteInput,
                    L"\\BaseNamedObjects\\Local\\SyncTranslateRemoteInput",
...
)
{
    ...
    else
    {
        ...
        // Read render DMA and forward PCM to SyncTranslate shared memory.
...
#if SYNC_BRIDGE_KERNEL_ENABLED
        (void)SyncBridgeReadIntoDma(m_pWfExt, m_pDmaBuffer + bufferOffset, runWrite);
#elif SYNC_DRIVER_PCM_RING_ENABLED
        SyncTranslatePcmRingReadIntoDma(m_pWfExt, m_pDmaBuffer + bufferOffset, runWrite);
#elif SYNC_TTS_INJECTION_ENABLED
        SyncTtsLoopbackReadIntoDma(m_pWfExt, m_pDmaBuffer + bufferOffset, runWrite);
#else
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
#if SYNC_BRIDGE_KERNEL_ENABLED
        (void)SyncBridgeWriteFromDma(m_pWfExt, m_pDmaBuffer + bufferOffset, runWrite);
#elif SYNC_TTS_INJECTION_ENABLED
        if (m_ulPin == SYNC_TTS_INJECTION_PIN)
        {
            SyncTtsLoopbackWriteFromDma(m_pWfExt, m_pDmaBuffer + bufferOffset, runWrite);
        }
#endif
